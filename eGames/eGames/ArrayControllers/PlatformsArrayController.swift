...
class PlatformsArrayController: NSArrayController {
    ...
    public func formatted() -> String {
        if platformset.count > 0 {
            let arr = Array(platformset).sorted(by: { $0.name < $1.name })
            var letter = arr[0].name.prefix(1).uppercased()
            var content : String = String("\(letter)\n")
            let padding = longest()
            for platform in arr {
                if platform.name.prefix(1).uppercased() != letter {
                    letter = platform.name.prefix(1).uppercased()
                    content += "\(letter)\n"
                }
                content += "\(platform.formatted(padding))\n"
            }
            return content
            }
        return String()
    ...
    public func exists(_ newPlatform : Platform) -> Bool {
        
        for platform in platformset {
            if newPlatform.name == platform.name &&
                dateToString(newPlatform.released) == dateToString(platform.released) &&
                newPlatform.developer == platform.developer &&
                newPlatform.manufacturer == platform.manufacturer &&
                newPlatform.generation == platform.generation &&
                newPlatform.type == platform.type {
