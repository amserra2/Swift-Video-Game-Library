...
public final class GamesArrayController: NSArrayController {
    ...
    public func formatted() -> String {
        if gameset.count > 0 {
            let arr = Array(gameset).sorted(by: { $0.title < $1.title })
            var letter = arr[0].title.prefix(1).uppercased()
            var content : String = String("\(letter)\n")
            let padding = longest()
            for game in arr {
                if game.title.prefix(1).uppercased() != letter {
                    letter = game.title.prefix(1).uppercased()
                    content += "\(letter)\n"
                }
                content += "\(game.formatted(padding))\n"
            }
            return content
        }
        return String()
    ...
    public func exists(_ newGame : Game) -> Bool {
        let x = titles(newGame.title.prefix(1).uppercased())
        
        for game in x {
            if newGame.title == game.title &&
                dateToString(newGame.released) == dateToString(game.released) &&
                newGame.platform == game.platform &&
                newGame.publisher == game.publisher &&
                newGame.format == game.format &&
                newGame.developer == game.developer &&
                newGame.genre == game.genre &&
                newGame.getMode() == game.getMode() {
                    selectedGame = game
                    return true
            }
        }
