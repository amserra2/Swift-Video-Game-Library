    ...
    @IBAction func done(_ sender: Any) {
        ...
        if or(.on) {
            ...
            if gamesButton.state == .on {
                string += "---Games---\n"
                string += viewController.gamesArrayController.downloadString()
                string += "\n"
            }
            if platformsButton.state == .on {
                string += "---Platforms---\n"
                string += viewController.platformsArrayController.downloadString()
                string += "\n"
            }
            if gamesWishlistButton.state == .on {
                string += "---Wishlist Games---\n"
                string += viewController.wishlistArrayController.downloadStringG()
                string += "\n"
            }
            if platformsWishlistButton.state == .on {
                string += "---Wishlist Platforms---\n"
                string += viewController.wishlistArrayController.downloadStringP()
                string += "\n"
            }
